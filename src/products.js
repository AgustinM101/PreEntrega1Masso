export let products = [
    {
      id: '1',
      name: 'Simulador de Vuelo',
      description: 'Un simulador de vuelo realista para pilotos en entrenamiento.',
      price: 299.99,
      image: 'https://res.cloudinary.com/dkv58dvqy/image/upload/v1738619823/simuVuelo_jpuypw.jpg',
      stock: 1,
      category: 'vuelo'
    },
    {
      id: '2',
      name: 'Simulador de Carreras',
      description: 'Experimenta la emoción de las carreras con este simulador.',
      price: 199.99,
      image: 'https://res.cloudinary.com/dkv58dvqy/image/upload/v1738619814/simuCarreras_tceqig.jpg',
      stock: 5,
      category: 'carreras'
    },
    {
      id: '3',
      name: 'Simulador de Espacio',
      description: 'Explora el espacio y más allá con este simulador.',
      price: 399.99,
      image: 'https://res.cloudinary.com/dkv58dvqy/image/upload/v1738619799/simuEspacio_lnaajp.avif',
      stock: 2,
      category: 'espacio'
    },
    {
      id: '4',
      name: 'Simulador de Shooters',
      description: 'Un simulador de shooters online para una experciencia unica.',
      price: 399.99,
      image: 'https://res.cloudinary.com/dkv58dvqy/image/upload/v1739934198/SimuShooters2_t7awfh.jpg',
      stock: 1,
      category: 'shooters'
    },
    {
      id: '5',
      name: 'Simulador de Shooters 2',
      description: 'Un simulador de shooters online para una experciencia unica.',
      price: 399.99,
      image: 'https://res.cloudinary.com/dkv58dvqy/image/upload/v1739934193/simuShooters_isfd7t.jpg',
      stock: 1,
      category: 'shooters'
    },
    {
      id: '6',
      name: 'Simulador de Vuelo 2',	
      description: 'Un simulador de volar para una experciencia unica.',
      price: 399.99,
      image: 'https://res.cloudinary.com/dkv58dvqy/image/upload/v1739934186/simuVolar_wmh2rv.jpg',
      stock: 2,
      category: 'vuelo'
    }

];

